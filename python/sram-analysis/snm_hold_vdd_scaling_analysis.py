...
def snm_hold_vdd_scaling_analysis():
    ...
    for scaling in np.arange(vdd_start, vdd_stop, -vdd_step):
        ...
        snm_gaussian_vth_hold_file_path = "snm_hold_vdd_scaling"
        vdd_scaled_str = str(vdd_scaled).replace('.', '')
        if vdd_scaled == 1:
            vdd_scaled_str = '1'
        snm_gaussian_vth_hold_file_path = os.path.join(f'{data}/out/hold/snm/vdd_scaling',
                                                       snm_gaussian_vth_hold_file_path + '_' + vdd_scaled_str + '.txt')
        ...
        snm_hold_array.append(snm_hold)

        print(f'snm_mean_gaussian_vth_hold[{vdd_scaled}V] = {snm_mean} mV', flush=True)
        print(f'snm_stdev_gaussian_vth_hold[{vdd_scaled}V] = {snm_stdev} mV')
