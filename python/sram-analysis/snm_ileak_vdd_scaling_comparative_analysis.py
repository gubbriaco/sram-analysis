...
def snm_ileak_vdd_scaling_comparative_analysis():
    ...
    print("{:<10} {:<25} {:<25} {:<55} {:<55}".format("VDD", "SNM(HOLD)", "SNM(READ)", "I_LEAK", "P_LEAK"))
    for (
            vdd,
            snm_hold_mean,
            snm_hold_stdev,
        ...
        pileak_str = f'({ileak_mean*vdd} W, {i_leak_stdev*vdd} W)'
        print("{:<10} {:<25} {:<25} {:<55} {:<55}".format(vdd_str, snm_hold_str, snm_read_str, ileak_str, pileak_str))


    fig_table_1, axs_table_1 = plt.subplots(1, 4, figsize=(16, 4))
    ...
    axs_table_1[3].plot(vdd_scaled, [ileak * vdd for ileak, vdd in zip(i_leak_standard_hold_mean, vdd_scaled)])
    axs_table_1[3].set_xlabel("vdd")
    axs_table_1[3].set_ylabel("p_leak")
    axs_table_1[3].legend(["p_leak"])
    axs_table_1[3].set_title("P_LEAK")
    axs_table_1[3].grid()
    ...
    axs_table_2_1.plot(
        vdd_scaled,
        snm_gaussian_vth_hold_mean,
        '-*', label='snm_hold', color='red'
    )
    axs_table_2_1.text(
        vdd_scaled[7],
        snm_gaussian_vth_hold_mean[7],
        'snm_hold'
    )
    axs_table_2_1.plot(
        vdd_scaled,
        snm_gaussian_vth_read_mean,
        '-*', label='snm_read', color='red'
    )
    axs_table_2_1.text(
        vdd_scaled[7],
        snm_gaussian_vth_read_mean[7],
        'snm_read'
    )
    ...
    axs_table_2_2.plot(
        vdd_scaled,
        i_leak_standard_hold_mean,
        '-*', label='i_leak', color='green'
    )
    axs_table_2_2.text(
        vdd_scaled[7],
        i_leak_standard_hold_mean[7],
        'i_leak'
    )
    axs_table_2_2.plot(
        vdd_scaled,
        [ileak * vdd for ileak, vdd in zip(i_leak_standard_hold_mean, vdd_scaled)],
        '-*', label='p_leak', color='green'
    )
    axs_table_2_2.text(
        vdd_scaled[7],
        [ileak * vdd for ileak, vdd in zip(i_leak_standard_hold_mean, vdd_scaled)][7],
        'p_leak'
    )
    axs_table_2_2.set_ylabel("i_leak, p_leak", color='green')
    axs_table_2_2.tick_params(axis='y', labelcolor='green')
    axs_table_2_2.legend(["i_leak", "p_leak"], loc='center left')
